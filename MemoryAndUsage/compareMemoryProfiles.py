...
- PDF + PNG output
- Fixed axes-frame position/size (figure-fraction), so the plot frame
  area is identical across figures regardless of tick-label width or
  tick count -- useful when comparing plots with different y ranges.
...
# Ensures the frame size is always the same
DEFAULT_AXES_RECT = (0.10, 0.10, 0.83, 0.85)
...
def plot_gpu_memory(
    ...
    # Fixed axes-frame position/size in figure-fraction coords:
    # (left, bottom, width, height). Keeping this constant across
    # figures (with a constant figsize) guarantees an identical frame
    # area regardless of tick-label width/count. Set to None to fall
    # back to plt.tight_layout() (frame size will then vary).
    axes_rect: tuple[float, float, float, float] | None = DEFAULT_AXES_RECT,
    # X axis
    ...
    axes_rect : tuple or None
        (left, bottom, width, height) in figure-fraction coordinates,
        used to pin the axes frame to a fixed position/size via
        ax.set_position(). Since this doesn't depend on tick-label
        width or tick count, all figures produced with the same
        figsize + axes_rect will have an identical plot frame area,
        which is what you want when comparing figures with different
        y-axis ranges. Pass None to instead use plt.tight_layout()
        (frame size will then vary figure to figure).
    xlims : tuple
    ...
    # Pin the axes frame to a fixed figure-fraction rect *before* any
    # further drawing. This must be done instead of (not in addition
    # to) plt.tight_layout(), since tight_layout would resize the
    # frame again based on label/tick extents.
    if axes_rect is not None:
        ax.set_position(axes_rect)

    max_time, min_time = -1., 1E9

    # Collect peak-memory annotations boxes here
    peak_annotations = []
    ...
    for file, legend_entry in zip(csv_files, legend_entries):
        ...
        for mem, label in curves:
            ...
            peak_annotations.append((max_mem, colour))
    ...
    true_xmin, true_xmax = ax.get_xlim()
    x_span = true_xmax - true_xmin
    x_positions = [true_xmax - 0.02 * x_span, true_xmax - 0.10 * x_span]

    # Vertical distance (in data/GiB units) below which two annotations
    # are considered close enough to overlap.
    overlap_threshold = 0.05 * (true_ymax - true_ymin)

    order = sorted(range(len(peak_annotations)), key=lambda i: peak_annotations[i][0], reverse=True)

    last_placed_y = None
    slot = 0
    for idx in order:
        max_mem, colour = peak_annotations[idx]
        if last_placed_y is not None and (last_placed_y - max_mem) < overlap_threshold:
            slot = 1 - slot  # alternate to the other horizontal slot
        else:
            slot = 0  # back to the default (rightmost) slot
        x_pos = x_positions[slot]

        ax.text(x_pos, max_mem, f"{max_mem:.1f} GiB",
                verticalalignment="center", horizontalalignment="right",
                color=colour, fontsize=9, fontweight="bold",
                bbox=dict(boxstyle="round,pad=0.2", facecolor="white", edgecolor=colour, alpha=0.8))

        last_placed_y = max_mem

    # NOTE: plt.tight_layout() is intentionally NOT called here when
    # axes_rect is set -- tight_layout would re-adjust (and thus vary)
    # the frame size based on label/tick extents, defeating the point
    # of pinning axes_rect. If axes_rect is None, fall back to
    # tight_layout so single, non-comparative plots still look good.
...
def main(args):
    fig, ax = plot_gpu_memory(
        ...
        per_gpu=False,
        figsize=(12, 9),
        axes_rect=tuple(args.axes_rect) if args.axes_rect else None,
        xlims=args.xlims,
        ylims=args.ylims,
        xlabel=args.xlabel,
        ylabel=args.ylabel,
    ...
    for ext in ('png', 'pdf'):
        ...
        fig.savefig(figname, dpi=300)  # no bbox_inches="tight": that
        # would recompute the bbox from label extents, resizing the
        # frame just like tight_layout does. Fixed axes_rect + fixed
        # figsize is what keeps the frame identical across figures.
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        nargs=4,
        required=False,
        default=list(DEFAULT_AXES_RECT),
        metavar=('LEFT', 'BOTTOM', 'WIDTH', 'HEIGHT'),
        help=(
            'Fixed axes-frame position/size in figure-fraction coordinates '
            '(left, bottom, width, height). Keeping this the same across runs '
            '(with the same figure size) guarantees an identical plot-frame area '
            'even when y-axis ranges/tick-label widths differ. Pass 0 0 0 0, or '
            'edit the script to use None, to fall back to automatic tight_layout '
            'sizing instead.'
        ),
    )
